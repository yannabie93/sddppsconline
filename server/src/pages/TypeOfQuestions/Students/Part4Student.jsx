...
import { publicRequest, students1Request } from '../../../utils/publicRequest';
...
const Part4Student = () => {
  ...
  //actual table names
  const actualTableName = getTable.split('questionpart4')[0]
  ...
  const handleDelete = async (e) => {
    try {
       await students1Request.delete(`/students4/${e.idstudentsquestionpart4}`)
  ...
  useEffect(() =>{
    const getData = async () =>{
      ...
      try {
        ...
        const res = await students1Request.get(`students4`)
      ...
      renderCell: (params) => {
        return (
          <Box sx={{ display: 'flex', flexDirection: 'row', alignItems: 'center', justifyContent: 'center', gap: 1 }}>
            ...
            <Link to={`edit/${params.row[`idstudentsquestionpart4`]}`}>
            ...
            <Link to={`choice/${params.row[`idstudentsquestionpart4`]}`}>
  ...
  return (
    ...
    <Box sx={{display:'flex', flex:2, p:4}}>
      <Box sx={{display:'flex', flexDirection:'column', gap:2, width: '100%'}}>
        ...
        <Typography variant='h5' color="text.disabled" sx={{fontWeight:"bold", textTransform: 'uppercase'}}>Student Questions Part 4</Typography>
