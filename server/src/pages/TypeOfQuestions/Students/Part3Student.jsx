...
const Part3Students = () => {
  ...
  const handleDelete = async (e) => {
    try {
       await students1Request.delete(`/students3/${e.idstudentsquestionpart3}`)
  ...
  useEffect(() =>{
    const getData = async () =>{
      ...
      try {
        ...
        const res = await students1Request.get('students3')
      ...
      renderCell: (params) => {
        return (
          <Box sx={{ display: 'flex', flexDirection: 'row', alignItems: 'center', justifyContent: 'center', gap: 1 }}>
            ...
            <Link to={`edit/${params.row[`idstudentsquestionpart3`]}`}>
            ...
            <Link to={`choice/${params.row[`idstudentsquestionpart3`]}`}>
  ...
  return (
    ...
    <Box sx={{display:'flex', flex:2, p:4}}>
      <Box sx={{display:'flex', flexDirection:'column', gap:2, width: '100%'}}>
        ...
        <Typography variant='h5' color="text.disabled" sx={{fontWeight:"bold", textTransform: 'uppercase'}}>Student Questions Part 3</Typography>
