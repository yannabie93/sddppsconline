...
const Part1Students = () => {
  ...
  const handleDelete = async (e) => {
    try {
       await students1Request.delete(`/students1/${e.idstudentsquestionpart1}`)
  ...
  useEffect(() =>{
    const getData = async () =>{
      ...
      try {
        ...
        const res = await students1Request.get('students1')
      ...
      renderCell: (params) => {
        return (
          <Box sx={{ display: 'flex', flexDirection: 'row', alignItems: 'center', justifyContent: 'center', gap: 1 }}>
            ...
            <Link to={`edit/${params.row[`idstudentsquestionpart1`]}`}>
            ...
            <Link to={`choice/${params.row[`idstudentsquestionpart1`]}`}>
  ...
  return (
    ...
    <Box sx={{display:'flex', flex:2, p:4}}>
      <Box sx={{display:'flex', flexDirection:'column', gap:2, width: '100%'}}>
        ...
        <Typography variant='h5' color="text.disabled" sx={{fontWeight:"bold", textTransform: 'uppercase'}}>Student Questions Part 1</Typography>
