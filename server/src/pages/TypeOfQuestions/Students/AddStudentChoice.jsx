...
import { students1Request } from '../../../utils/publicRequest'
import { rows } from '../../../utils/sampleData'
import BeatLoader from "react-spinners/BeatLoader";

const AddStudentChoice = () => {
 ...
 const initialState = {
  ...
  const handleDelete = async (e) => {
    console.log(e)
    try {
       await students1Request.delete(`/choices/${e}`)
       console.log("Delete Successfuly")
       setGetChoices(prevState =>
        prevState.filter(row => row.idstudentschoices !== id)
      );
    } catch (error) {
      console.log("Error")
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      await students1Request.post('/choices', AddChoice)
  ...
  useEffect(() => {
    const getSingleQuestion = async () => {
      try {
        ...
        const choice = await students1Request.get(`/choices/${id}?part=studentsquestionpart${getTablePart}`);
            ...
            <TableContainer component={Paper}>
              <Table sx={{ minWidth: 650 }} size="small" aria-label="a dense table">
                ...
                <TableBody>
                  {getChoices.map((row) => (
                    <TableRow
                      key={row.idstudentschoices}
                      sx={{ '&:last-child td, &:last-child th': { border: 0 } }}
                    >
                      <TableCell align='left'>{row.question_choices}</TableCell>
                      <TableCell align='right'>
                        <Button variant="outlined" onClick={() =>handleDelete(row.idstudentschoices)} color="error">Delete</Button>
                      </TableCell>
...
export default AddStudentChoice