...
const Part2Employee = () => {
  ...
  const handleDelete = async (e) => {
    try {
       await employees1Request.delete(`/employees2/${e.idemployeesquestionpart2}`)
  ...
  useEffect(() =>{
    const getData = async () =>{
      ...
      try {
        ...
        const res = await employees1Request.get(`employees2`)
      ...
      renderCell: (params) => {
        return (
          <Box sx={{ display: 'flex', flexDirection: 'row', alignItems: 'center', justifyContent: 'center', gap: 1 }}>
            ...
            <Link to={`edit/${params.row[`idemployeesquestionpart2`]}`}>
            ...
            <Link to={`choice/${params.row[`idemployeesquestionpart2`]}`}>
  ...
  return (
    ...
    <Box sx={{display:'flex', flex:2, p:4}}>
      <Box sx={{display:'flex', flexDirection:'column', gap:2, width: '100%'}}>
        ...
        <Typography variant='h5' color="text.disabled" sx={{fontWeight:"bold", textTransform: 'uppercase'}}>Employee Questions Part 2</Typography>
