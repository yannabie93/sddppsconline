...
const Part3Faculty = () => {
  ...
  const handleDelete = async (e) => {
    try {
       await faculty1Request.delete(`/faculty3/${e.idfacultyquestionpart3}`)
  ...
  useEffect(() =>{
    const getData = async () =>{
      ...
      try {
        ...
        const res = await faculty1Request.get(`faculty3`)
      ...
      renderCell: (params) => {
        return (
          <Box sx={{ display: 'flex', flexDirection: 'row', alignItems: 'center', justifyContent: 'center', gap: 1 }}>
            ...
            <Link to={`choice/${params.row[`idfacultyquestionpart3`]}`}>
  ...
  return (
    ...
    <Box sx={{display:'flex', flex:2, p:4}}>
      <Box sx={{display:'flex', flexDirection:'column', gap:2, width: '100%'}}>
        ...
        <Typography variant='h5' color="text.disabled" sx={{fontWeight:"bold", textTransform: 'uppercase'}}>Faculty Questions Part 3</Typography>
