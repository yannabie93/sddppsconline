...
const Part2Faculty = () => {
  ...
  const handleDelete = async (e) => {
    try {
       await faculty1Request.delete(`/faculty2/${e.idfacultyquestionpart2}`)
  ...
  useEffect(() =>{
    const getData = async () =>{
      ...
      try {
        ...
        const res = await faculty1Request.get(`faculty2`)
      ...
      renderCell: (params) => {
        return (
          <Box sx={{ display: 'flex', flexDirection: 'row', alignItems: 'center', justifyContent: 'center', gap: 1 }}>
            ...
            <Link to={`choice/${params.row[`idfacultyquestionpart2`]}`}>
      ...
      <Box sx={{display:'flex', flex:2, p:4}}>
        ...
        <Box sx={{display:'flex', flexDirection:'column', gap:2, width: '100%'}}>
          ...
          <Box sx={{display:'flex', alignItems:'center', justifyContent:'space-between', width: '100%'}}> 
            <Typography  variant='h5' color="text.disabled" sx={{fontWeight:"bold", textTransform: 'uppercase'}}>Faculty Questions Part 2</Typography>
