...
const Part4Faculty = () => {
  ...
  const handleDelete = async (e) => {
    try {
       await faculty1Request.delete(`/faculty4/${e.idfacultyquestionpart4}`)
  ...
  useEffect(() =>{
    const getData = async () =>{
      ...
      try {
        ...
        const res = await faculty1Request.get(`faculty4`)
      ...
      renderCell: (params) => {
        return (
          <Box sx={{ display: 'flex', flexDirection: 'row', alignItems: 'center', justifyContent: 'center', gap: 1 }}>
            ...
            <Link to={`edit/${params.row[`id${getTable}`]}`}>
            ...
            <Link to={`choice/${params.row[`idfacultyquestionpart4`]}`}>
  ...
  return (
    ...
    <Box sx={{display:'flex', flex:2, p:4}}>
      <Box sx={{display:'flex', flexDirection:'column', gap:2, width: '100%'}}>
        ...
        <Typography variant='h5' color="text.disabled" sx={{fontWeight:"bold", textTransform: 'uppercase'}}>Faculty Questions Part 4</Typography>
